...
def _(mo):
    ...
    mo.md(f"```python\n{predictor_code}\n```")
    return f, predictor_code


@app.cell(hide_code=True)
def _(mo):
    mo.md("### `src/app.py` - FastAPI サーバ")
    return
...
def _(mo):
    ...
    mo.md(f"```dockerfile\n{dockerfile_code}\n```")
    return dockerfile_code, f3


@app.cell(hide_code=True)
def _(IMAGE_URI, MODEL_GCS_URI, mo):
    mo.md(
        f"""
        ### Docker ビルドと起動
...
def _(ENDPOINT_ID, aiplatform, base64, mo):
    ...
    return endpoint, image_b64, img_f, pathlib, response, sample_image_path
...
def _(ENDPOINT_ID, PROJECT_ID, REGION, base64, mo):
    ...
    return Image, ImageDraw, demo, detect, draw_detections, np
