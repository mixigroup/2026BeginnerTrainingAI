...
- インフラ、データベース、サーバ、プログラムなど
- ChatGPTのようなサービス全体

### 機械学習 (Machine Learning)

- 大量のデータをもとに文章を生成するように学習すること
...
- **盤面のスコアリングから、次の行動へ → 盤面の差の高い角をも〜にしよう**

### 機械学習：過去のデータの中からルールを得る

- 大量の棋譜からパターンを見つけて、次の手を決定する
- **盤面の画像から、差し手のパターンから角をも〜にしよう**
...
実は教師なし学習の他にもあ理ますが、今回の研修は理解しやすい **教師あり** を軸に進めます
...
**ゴール**: ノートブックの推論を「他の人/他のサービスが呼べる形」にする。

### やること

- FastAPI で `/predict` エンドポイントを作る
- 事前処理 → forward → 後処理 を API 内に入れる
- Streamlit / Gradio から API を呼んでデモ画面を作る

### 確認ポイント

- 入出力スキーマが固定されているか（型、サイズ、エラー時の返し方）
- ログに「入力の概要」と「推論結果」と「レイテンシ」が残るか

---

# 拡張：Vertex AI へデプロイ（可能なら）

**狙い**: マネージド環境でスケール/運用を簡単にする。
